<?php include '_header.php'; ?>
<!-- Page Content -->
<?php include 'check_admin_level.php'; ?>

<!-- Content Head -->
<div id="page-wrapper">
    <div class="container-fluid">
        <div class="row bg-title">
            <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
            <h3 class="page-title">Movies</h3> </div>
            <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                <ol class="breadcrumb">
                    <li><a href="#">Dashboard</a></li>
                    <li class="active">Movies</li>
                </ol>
            </div>
        </div>
        <!-- Content Head end-->
        <!-- Content Body -->
        <div class="row">
            <div class="col-md-12">
                <div class="white-box">
                    <h3 class="box-title">Movies <a href="newmovie.php" class="btn btn-info pull-right">New Movies</a></h3> <br>
                    <?php
                    $getadminsql = mysql_query("select * from tbl_movie;");
                    ?>
                    <table class="table table-hover table-striped table-responsive" id="table">
                        <thead>
                            <tr>
                                <th>ID</th>
                                <th>Movie Name</th>
                                <th>Type</th>
                                <th>Released Date</th>
                                <th>Runtime</th>
                                <th>Genre</th>
                                <th>Price</th>
                                <th>Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php while ($result = mysql_fetch_array($getadminsql)){  ?>
                            <tr>
                                <td><?php echo $result['id']; ?></td>
                                <td><?php echo $result['name']; ?></td>
                                <td><?php echo $result['type']; ?></td>
                                <td><?php echo $result['released_date']; ?></td>
                                <td><?php echo $result['runtime']; ?></td>
                                <td><?php echo $result['genre']; ?></td>
                                <td><?php echo $result['price']; ?></td>
                                <td><a href='viewmovie.php?id=<?php echo $result['id'];?>' class="btn btn-primary btn-xs"  >View</a></td>
                            </tr>
                            <?php } ?>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
        <script>
        $('#table').dataTable(
        );
        </script>
        <!-- Content Body end -->
    </div>
<footer class="footer text-center"> 2018 &copy; Codex Admin Team </footer>
</div>
</div>
</body>
</html>